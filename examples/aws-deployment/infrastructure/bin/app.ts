...
// How the EC2 worker authenticates to GitHub. A personal access token is the
// default and what nearly every deployment uses; a GitHub App is the option for
// organisations that require one -- registering and installing an App needs
// org-admin rights, which many adopters do not have.
//
// Exactly one of the two: CanopyCmsService refuses both at synth, because two
// credentials leave it undefined which identity a push or a PR acts as. So
// GITHUB_TOKEN_SECRET_ARN is `required()` -- an unset one must still fail loudly
// -- EXCEPT when the App variables are set, where there is no token to point at.
//
// ANY of the three, not just the app id. Someone who sets two of the three and
// forgets the third has plainly chosen the App path, and gating on the app id
// alone would answer them with "GITHUB_TOKEN_SECRET_ARN must be set" -- telling
// them to restore the credential they were just told to remove. Letting the
// partial set through instead reaches CanopyCmsService's own guard, which names
// the variable that is actually missing.
const usingGitHubApp = Boolean(
  process.env.GITHUB_APP_ID ||
  process.env.GITHUB_APP_INSTALLATION_ID ||
  process.env.GITHUB_APP_PRIVATE_KEY_SECRET_ARN,
)
...
new CmsStack(app, 'CanopyCms', {
  ...
  githubTokenSecretArn: usingGitHubApp
    ? process.env.GITHUB_TOKEN_SECRET_ARN || undefined
    : required('GITHUB_TOKEN_SECRET_ARN'),
  clerkSecretKeySecretArn: required('CLERK_SECRET_KEY_SECRET_ARN'),

  // Optional, and NOT `required()`: GitHub App authentication, as an
  // alternative to the token above. Set all three or none -- an installation
  // token is minted from all three together, and a partial set is refused at
  // synth. The private key is ARN-only: it is a multi-line PEM, and every value
  // the construct puts in the worker's environment must be single-line.
  ...
  githubAppPrivateKeySecretJsonField:
    process.env.GITHUB_APP_PRIVATE_KEY_SECRET_JSON_FIELD || undefined,
