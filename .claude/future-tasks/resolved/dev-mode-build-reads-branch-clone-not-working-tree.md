...
applied inside `loadOrCreateBranchContext`. A local `next build` reads what is on disk,
uncommitted files included; CI builds the checked-out commit on a detached HEAD, and example1's
`git checkout -B main` step is gone. Editor saves not yet copied out by `canopycms sync pull` are
not part of a build. Left open:
[build-canopy-scripts-outside-next-build.md](../build-canopy-scripts-outside-next-build.md). The
text below is the original report.
