...
| Phantom-dirty Save on every load; false "Unsaved Changes" modal on branch create | Save now deletes the localStorage draft after persisting; entry load always fetches the server value (draft becomes a true overlay), so dirty-tracking is truthful. Commit `e43b7a6`. Also resolves [[stale-draft-prevents-content-load]]. |
...
- **Unresolve for resolved comment threads**: needs a new comment-store primitive + API route — captured as [[comment-thread-unresolve]].
