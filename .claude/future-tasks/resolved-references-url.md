...
Tagged **[KB]**: the workaround this removes is the knowledge base site's
own, and it deploys first.
...
The knowledge base site built its own doc-link index helper — a **second
full `listEntries` pass** over the entire content tree, purely to build a
...
Once this ships, the doc-link index helper can be deleted outright (it
becomes strictly redundant with what a resolved reference already carries),
and the page-wide `resolveReferences: false` escape hatch it forced becomes
