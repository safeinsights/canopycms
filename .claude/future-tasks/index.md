...
| [entry-create-modal-slug-reset-race.md](entry-create-modal-slug-reset-race.md) | Found 2026-07-30 writing media-upload.spec.ts: `EntryCreateModal`'s slug reset effect is keyed on the `entryTypes` array identity, so a stray parent re-render between typing a slug and submitting silently reverts it to `untitled` — entry lands on disk under the wrong filename with no error (~1 in 8 under suite render pressure) |
