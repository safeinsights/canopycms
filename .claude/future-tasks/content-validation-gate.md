...
task file covered this. Tagged **[KB]** rather than [BOTH]: the KB already
...
rendering pipeline, in production, in front of a reader. `docs-site-proto`
was hit by this in a real production incident and responded by writing its
own defense: `scripts/validate-mdx.ts` + `src/lib/mdx-render-check.ts`, a
standalone script that actually renders each entry's content and catches
what schema validation cannot.
...
- The adopter-side build gate and render-check module this ports from: a
  ...
  Next server graph. Ask the KB maintainers for the current source.
