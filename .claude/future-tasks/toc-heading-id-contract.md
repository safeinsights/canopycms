...
Both sites independently wrote `src/lib/rehype-heading-attrs.ts` to stamp
stable `id`s onto rendered headings — a real, converged signal that this
belongs in the package, not adopter code.

`website` went further and layered `toc.ts` on top to extract a table of
contents from the annotated headings, but it **hand-mirrors rehype-slug's
dedup counter** (the "if this slug already exists, append `-1`, `-2`, …"
