...
3. **It still wouldn't solve the problem.** The request is driven by two
   sites' divergent MDX rendering pipelines (the KB has one MDX-evaluation
   pipeline with its own doc-page/doc-view components and a render-safety
   check; the marketing site has two separate pipelines), and a
   plain-markdown renderer covers none of the MDX half — which is the half
   that's actually diverging.
...
- That CanopyCMS doesn't ship an opinionated renderer on purpose, and points
  at in-repo examples (`apps/example1`'s markdown handling, and an adopter's
  own MDX-evaluation pipeline) as reference shapes rather than prescriptions.
