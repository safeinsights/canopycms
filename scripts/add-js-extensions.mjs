...
//
// Applies to BOTH the emitted .js and the emitted .d.ts. The .d.ts half fails
// silently: on moduleResolution "node16"/"nodenext" an unresolved specifier
// degrades the whole import to `any` instead of erroring, so the adopter's
// build stays GREEN while our types vanish -- visible as a TS2834/TS2835
// diagnostic only under skipLibCheck:false, invisible under the
// skipLibCheck:true most scaffolds (Next.js included) set. Appending `.js` is
// still correct for a .d.ts specifier: TypeScript resolves `./x.js` to
// `./x.d.ts`, so declaration files must name the runtime extension, never
// `.d.ts`.
//
// Caveat: this is a textual rewrite and cannot tell a specifier from prose, so
// an `import`-shaped string in a comment or JSDoc @example gets the same
// treatment. Harmless -- it changes no resolution -- but adopter-visible in
// IntelliSense.
//
// Shared by every published package's build (ONE implementation): each
// package invokes it against its own dist/ after tsc emits it, either via the
// CLI form below or by importing `addJsExtensions` directly. canopycms's
// extra postbuild steps (CLI template-file copying, tsx-shebang stripping,
// esbuild bundling) stay in packages/canopycms/scripts/postbuild.mjs, which
// imports this rather than duplicating it.
...
// security/detect-unsafe-regex flags star height alone: `[^'"]*` sits inside
// the optional `(?:\/...)?` group, which matches at most once, so there is no
// nested repetition to backtrack through -- linear, not exponential.
// Suppressed at the line rather than disabling the rule for the directory, so
// a genuinely unsafe pattern added later still trips it.
...
export async function addJsExtensions(dir) {
  ...
  for (const entry of entries) {
    ...
    const rewritten = original.replace(RELATIVE_IMPORT_RE, (match, prefix, specifier, quote) => {
      ...
      // a bare '.' — operating-mode/types.ts has the one occurrence in this
      // repo, and only the .d.ts half of this rewrite catches it.
...
// specifier (a bare `.`, say) ships unrewritten, too wide and a bare package
// name gets a spurious `.js` welded onto it. Neither shows up
// as a build error.
