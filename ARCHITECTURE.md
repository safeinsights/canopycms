...
The single source of truth is `getBranchProtection(config, branchName, recordedBaseBranch?, status?)` in `authorization/protected-branch.ts`, keyed off the resolved `config.defaultBaseBranch` (never a hard-coded `'main'` — `master`/`develop` bases work) with sanitization-aware comparison (metadata names are sanitized; config holds the raw git name). It returns four flags:
...
`writeBlocked` is the only flag that also depends on the optional `status` argument: it is `readOnly || (status !== undefined && status !== 'editing')`. This is what makes the function the single source of truth for **both** reasons a branch refuses writes — base-branch protection and the review lock (see [Review Lock](#branch-workflow)) — so that rule is expressed once rather than re-derived per call site. Callers that only care about base-branch protection omit `status`, and `writeBlocked` then simply mirrors `readOnly`. `readOnly` deliberately keeps its narrow meaning: it is what distinguishes _which_ lock applies, and therefore which banner the editor shows.
