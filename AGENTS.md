...
- `static/` - Framework-agnostic static-generation helpers: `collectStaticPaths` (paths only) and `collectRoutableEntries` (same enumeration, `data`/`updatedAt` carried through) share one internal pass, so both inherit the build-time schema-validity guard — sitemap generation is therefore a new place a production build can go red; `seo.ts` - pure `extractSeoFields` (empty-string-is-unset, because Canopy stores optional fields present-but-empty) plus `isNoindexEntry`, THE single `noindex` predicate that must feed both advertising surfaces (`robots: {index:false}` and sitemap exclusion) — deriving them separately is what let a noindex entry stay in one surface while absent from the other; enumeration must never filter on it, since noindex means "don't advertise", not "don't build". `resolveSeoUrl` checks absolute-URL-ness BEFORE normalizing (the reverse order rewrites an off-site canonical into `<siteUrl>/https://…/`). Next adapter: `canopycms-next/src/static.ts`'s `generateContentSitemap`/`entryToMetadata`, both bound onto `NextCanopyContextResult`. `trailingSlash` is an explicit option on both — Canopy has no knowledge of `next.config`
