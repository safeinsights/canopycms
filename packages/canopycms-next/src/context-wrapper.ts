...
export async function createNextCanopyContext(options: NextCanopyOptions) {
  ...
  const services = await createCanopyServices(options.config, { entrySchemaRegistry: options.entrySchemaRegistry })
  ...
  const extractUser = async (): Promise<CanopyUser> => {
    ...
    const mainBranchContext = await loadBranchContext({ branchName: baseBranch, mode: operatingMode })
    const internalGroups = mainBranchContext
      ? await loadInternalGroups(mainBranchContext.branchRoot, operatingMode, services.bootstrapAdminIds).catch(() => [])
      : []
