...
if (process.env.NODE_ENV === 'production') {
  throw new Error('canopycms-auth-dev: This plugin is for development only and cannot be used in production')
}
```
  ...
  <Stack>
    {users.map(user => (
      ...
      >
        ...
        <Group gap="xs">
          {user.groups?.map(g => <Badge key={g} variant="outline" size="sm">{g}</Badge>)}
        </Group>
      </Paper>
    ))}
  </Stack>
