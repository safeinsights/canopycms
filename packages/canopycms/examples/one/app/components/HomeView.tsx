...
export const HomeView: React.FC<{ data: HomeContent }> = ({ data }) => {
  const { data: liveData, highlightEnabled, fieldProps } = useCanopyPreview<HomeContent>({
    initialData: data,
  })
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-3 text-base leading-relaxed text-slate-600 whitespace-pre-wrap" {...fieldProps('hero.body')}>
          {hero.body}
        </p>
        <a
          href={cta.link || '#'}
