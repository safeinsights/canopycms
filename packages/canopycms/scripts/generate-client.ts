...
const HAND_WRITTEN_METHODS: Record<string, (route: RouteMetadata) => string> = {
  'assets.uploadProxied': (
    route,
  ) => `    /** ${route.method} ${route.path}: multipart upload, hand-written because the generic template only knows JSON bodies. */
...
${methods}
  ...
  const factories = Array.from(factoryResponseTypes)
    .map((responseType) => {
      ...
      return `export function ${funcName}(): ${responseType} {
