  ...
  private async request<T = unknown>(
    ...
    if (body !== undefined) {
      if (body instanceof FormData) {
        ...
        // NOTE: FormData bodies cannot carry an x-amz-content-sha256 hash (the
        // multipart boundary is generated at send time), so requests with a
        // FormData body will 403 behind the CloudFront OAC prod shape. See
        ...
        // CloudFront's Origin Access Control signs origin requests but never
        // hashes the payload itself - the viewer request must already carry
        // x-amz-content-sha256 or the OAC-protected Lambda Function URL
        // rejects the request with 403. Harmless on non-AWS deployments.
