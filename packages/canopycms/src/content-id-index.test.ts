...
import { unsafeAsLogicalPath, unsafeAsEntrySlug, unsafeAsPhysicalPath, unsafeAsContentId } from './paths/test-utils'
...
describe('ContentIdIndex', () => {
  ...
  describe('updatePath', () => {
    it('updates path for existing ID', () => {
      ...
      // Update path
      index.updatePath(unsafeAsContentId(testId), unsafeAsPhysicalPath(`content/new.${testId}.json`))
    ...
    it('throws for non-existent ID', () => {
      expect(() => index.updatePath(unsafeAsContentId('nonexistent'), unsafeAsPhysicalPath('content/new.json'))).toThrow(
  ...
  describe('getEntriesInCollection', () => {
    ...
    it('returns all entries in a collection', async () => {
      ...
      await fs.writeFile(
        path.join(tempDir, `content/posts/post.hello.${post1Id}.json`),
        '{}'
      )
      await fs.writeFile(
        path.join(tempDir, `content/posts/post.world.${post2Id}.json`),
        '{}'
      )
    ...
    it('does not include collection directories in results', async () => {
      ...
      const collectionId = 'c1L2L3e4c5t6'
      await fs.mkdir(
        path.join(tempDir, `content/posts.${collectionId}`),
        { recursive: true }
      )
    ...
    it('maintains index consistency after updatePath with collection change', () => {
      ...
      index.updatePath(unsafeAsContentId(entryId), unsafeAsPhysicalPath(`content/pages/page.article.${entryId}.json`))
    ...
    it('handles nested collections correctly', async () => {
      ...
      await fs.writeFile(
        path.join(tempDir, `content/docs/api/doc.intro.${apiEntryId}.json`),
        '{}'
      )

      const docsEntryId = 'd1c2s3e4n5t6'
      await fs.writeFile(
        path.join(tempDir, `content/docs/doc.guide.${docsEntryId}.json`),
        '{}'
      )
