...
describe('Permission Denied Errors', () => {
  ...
  it('returns 403 when reviewer tries to edit content', async () => {
    ...
    const readResponse = await reviewerClient.get('/api/canopycms/feature-reviewer-test/content/posts/test-post')
    ...
    const writeResponse = await reviewerClient.put('/api/canopycms/feature-reviewer-test/content/posts/test-post', {
      format: 'mdx',
      data: {
