...
describe('Settings Branch Isolation', () => {
  ...
  it('reads permissions from settings branch, not from main branch', async () => {
    ...
    const settingsBranchName = strategy.getSettingsBranchName({ settingsBranch: 'canopycms-settings' })
  ...
  it('verifies settings branch isolation works consistently across modes', async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        ...
        updatedAt: new Date().toISOString(),
        updatedBy: 'test',
        pathPermissions: [{ path: unsafeAsPermissionPath('content/**'), read: { allowedUsers: [user.userId] } }],
      })
    )
