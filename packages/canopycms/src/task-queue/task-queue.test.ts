...
describe('Task Queue', () => {
  ...
  describe('complete', () => {
    it('moves task to completed/ with result and timestamp', async () => {
      ...
      const task = JSON.parse(await fs.readFile(path.join(tmpDir, 'completed', `${id}.json`), 'utf-8'))
  ...
  describe('retry', () => {
    it('moves task back to pending with incremented retryCount', async () => {
      ...
      const task = JSON.parse(await fs.readFile(path.join(tmpDir, 'pending', `${id}.json`), 'utf-8'))
      expect(task.status).toBe('pending')
