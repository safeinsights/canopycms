  ...
  private async rebaseActiveBranches(): Promise<void> {
    ...
    for (const branchDir of branchDirs) {
      ...
      try {
        ...
        if (branchStatus === 'submitted' || branchStatus === 'approved' || branchStatus === 'archived') {
        ...
        if (!completed) {
          console.warn(`  Rebase of ${branchDir} did not complete within ${MAX_ROUNDS} rounds, aborting`)
          await branchGit.rebase(['--abort']).catch(() => {})
          continue
        }
