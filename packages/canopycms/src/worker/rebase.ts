...
/**
 * The rebase loop: the deepest leaf of the git-sync cluster, and the most
 * safety-critical part of the worker. `syncGit` (git-sync.ts) calls
 * `runRebaseCycle` once per cycle; every branch workspace behind the base
 * branch is rebased onto it, keeping the BRANCH's version of any conflicting
 * file. Three things make it the part most likely to misbehave under real EFS
 * and spot conditions, all documented at the point they happen:
 *
...
/**
 * Per-cycle outcome of `rebaseActiveBranches()`, folded by `syncGit()` into the
 * worker's self-reported status (`WorkerStatusReport.lastGitSync`, see
 * worker-status.ts) alongside a `durationMs` for the whole sync cycle.
 */
export interface RebaseSummary {
  /**
   * Branches that were behind and completed a rebase onto the base branch,
   * whether or not conflicts were resolved via --theirs. Branches already up to
   * date are NOT listed here.
   */
  rebased: string[]
...
/**
 * Poll GitHub for a submitted/approved branch's PR resolution. Those branches
 * sit outside the rebase loop and get no other signal that their PR resolved --
 * nothing pushes a merge/close webhook back into the branch workspace.
 *
 * merged -> auto-archive via buildMergedBranchUpdate, shared with the manual
 * markAsMerged API so both paths produce identical archived-branch metadata.
 * closed-without-merge -> record pullRequestState only; an admin decides the
 * workflow transition. Best-effort: a failure is logged, swallowed and retried
 * next sync cycle.
 */
export async function pollMergeState(
  ...
  try {
    ...
    if (data.merged) {
      ...
      // GitHub's actual merge time when available; buildMergedBranchUpdate
      // falls back to "now" when merged_at is absent.
      await meta.save({
    ...
    // Re-load fresh, not the `metaFile` snapshot passed in: a concurrent Lambda
    // write (an editor re-submitting) may have landed since it was taken.
...
/**
 * Persist a per-branch rebase failure to branch.json, bounded to roughly one
 * save per failing branch per hour: a branch stuck failing every cycle must not
 * become save-per-cycle x N-failing-branches write amplification, since save()
 * eager-regenerates the branch registry (branch-metadata.ts's
 * invalidateRegistry(), O(branch count) EFS reads).
 *
 * Best-effort and non-fatal, like every metadata write in this loop's error
 * paths: a corrupt branch.json, lock contention or any other save failure must
 * never abort the per-branch iteration. One of the two call sites is inside the
...
export async function recordRebaseFailure(
  ...
  // [REDACT] Defense in depth: rebaseFailure.message is persisted to
  // branch.json and served to the browser by the branch-health admin endpoint.
  // Both call sites already redact before passing in, but redactCredentials is
  // idempotent, so redacting here is free and keeps this method safe alone.
  ...
  try {
    ...
    // Includes BranchMetadataCorruptError from the load above (a save() against
    // the same corrupt file would just throw again): recording is best-effort
 ...
 *
 * ABORT OWNERSHIP is split across FOUR sites, none redundant; "the caller owns
 * the abort" would be wrong in a way that invites deleting a live one:
 ...
 *   only two the caller's `!completed` abort actually covers;
 * - a THROW (`branchGit.status()` is the first statement of the round catch, so
 *   a vanished `.git` or an EFS error escapes, as can the test hook beside it)
 *   unwinds to `rebaseOneBranch`'s outer catch, where the last-resort abort in
 *   its `finally` is the backstop -- not dead code.
 * (A fifth abort, in the interrupted-rebase recovery, cleans up a PREVIOUS run.)
 *
 * What this function DOES guarantee is narrower: no per-file resolution failure
 * escapes it. `checkout --theirs` on a MODIFY/DELETE conflict throws straight
 * out of the round loop, skipping both abort sites and wedging the clone
 * forever, so those set `failureReason` and break instead.
 *
...
async function runRebaseRounds(
  ...
  let completed = false
  // Captured only on the "unexpected error" exit below, for the failed-summary
  // entry pushed at the `if (!completed)` check.
  let failureReason: string | undefined

  for (let round = 0; round < MAX_REBASE_ROUNDS && !completed; round++) {
    // Re-checked every round: the compromise can land between rounds, and
    // `--continue`/`--skip` are as destructive as the initial rebase. Handled
    // after the loop so it cannot be mistaken for the `!completed`
    // rebase-FAILURE path below.
    ...
    try {
      ...
      if (st.conflicted.length > 0) {
        ...
        // During rebase, --theirs = the branch being replayed (the editor's
        // work): git rebase reverses ours/theirs, "ours" being the target.
        //
        // MODIFY/DELETE conflicts have no "their version" to check out and must
        // be resolved by staging a delete or an add instead. `git checkout
        // --theirs` on one exits non-zero and simple-git throws -- and because
        // this loop body IS the round loop's catch, that throw escapes the
        // round loop entirely, skipping BOTH `rebase --abort` sites below and
        // wedging the clone mid-rebase forever. The index/working-tree code
        // pair identifies which side deleted (verified against real git):
        //
        //   U/D  "deleted by them"  -- the BRANCH deleted it, base modified it,
        //        and git leaves base's version in the tree. Keeping the branch
        //        version means honouring the delete: git rm.
        //   D/U  "deleted by us"    -- base deleted it, the BRANCH modified it,
        //        and git leaves the branch's version in the tree. Keeping the
        //        branch version means keeping it: git add.
        //
        ...
        if (resolutionFailure !== undefined) {
          ...
          // failureReason and break, letting the `!completed` block do the
          // single `rebase --abort` and record the failure once.
        ...
        if (
          ...
          // Unexpected error — abort and leave the branch behind.
          // conflictStatus/conflictFiles are deliberately not updated: the
          // rebase did not complete, so the true conflict state is unknown, and
          // the existing (possibly stale) metadata is preserved until the next
          // successful rebase cycle corrects it.
 ...
 * shows as conflicted, deduplicated. IDs rather than paths because they are
...
export function conflictFilesToContentIds(
  ...
  // Entry files carry their ID in the filename. A `.collection.json` has none
  // of its own (extractIdFromFilename returns null for dot-prefixed files), so
  // the ID comes from the parent directory; the root content directory has no
  // embedded ID at all, so ROOT_COLLECTION_ID stands in -- but only for the
  // configured contentRoot.
  //
  // Two different notions of "parent" are needed, and conflating them is the
  // bug this guards against (see schema-store.ts's contentRootName doc comment
  // for the same shape elsewhere):
  //  - `parentDir` (a basename) recovers a SUB-collection's own embedded ID,
  //    correct as a basename since a collection directory carries its ID in its
  //    own name, one path segment;
  //  - `parentPath` (the full normalized relative parent path) is what must be
  //    compared against `contentRoot`, which is documented (config/helpers.ts)
  //    as allowed to span multiple segments (e.g. "cms/content"). Comparing a
  //    basename against that full value is always false, silently dropping the
  //    root collection's conflict. Git reports POSIX paths and the configured
  //    value may use either separator, so both sides go through
  //    normalizeFilesystemPath first.
 ...
 * the GitHub hop.
 *
 * Only runs when the pre-rebase history had ALREADY been published
 * (`publishedSha !== null`); otherwise nothing downstream knows about the
 * commits the rebase replaced. Without it, the editor's next submit is rejected
 * non-fast-forward forever.
 *
 * `preRebaseHead` and `publishedSha` must BOTH be read before the rebase -- the
 * clone's pre-rebase tip is exactly what the rebase destroys.
 */
async function carryForwardRewrittenHistory(
  ...
  if (publishedSha !== null) {
    if (publishedSha === preRebaseHead) {
      // ARMING GUARD: remote.git holds EXACTLY what this clone just rebased
      // away and nothing more, so a lease keyed to it can only undo our own
      // rewrite.
      //
      // The inequality case below is not defensive padding. Branch clones never
      // fetch their own branch (GitManager's clone is --single-branch and
      // checkoutBranch only checks out an existing local branch), while
      // reconcileTrackedBranches fast-forwards remote.git to GitHub's tip, so
      // after a direct push of a fixup to the PR branch remote.git legitimately
      // holds a commit this clone has never seen. Leasing on "whatever
      // remote.git currently holds" would be SATISFIED there and would silently
      // delete that fixup from remote.git and then GitHub. Keying the lease to
      // the pre-rebase tip turns that case into the visible divergence it is.
      //
      // Order matters: mark, then push, then queue. A crash after any step
      // leaves the marker set with the work unfinished, which
      // reconcilePendingRewrite() completes on a later cycle. Pushing first
      // would leave remote.git rewritten, GitHub stale and nothing recorded --
      // unrecoverable, and diagnosed as a false "another deployment" collision.
...
/**
 * What one branch contributed to the cycle summary. A return value rather than
...
export type BranchRebaseOutcome =
  ...
  | {
       ...
       * [SYNC-H1] carry-forward block, the sole code that runs after that point
       * and can still throw (via markHistoryRewritten's metadata save).
       *
       * Not a tidying-up detail: a branch genuinely belongs in BOTH buckets
       * there, because its history moved and publishing the move failed, and
       * collapsing it to `failed` alone would drop a real rebase from
       * worker-status.json.
       *
       * KNOWN COVERAGE GAP: only the `false` direction is exercised. Reaching
       * `true` needs `markHistoryRewritten`'s metadata save to fail AFTER a
       * completed rebase, and there is no seam for that -- the save sits
       * between two other saves with no hook between them, and adding a
       * production hook purely to reach it would cost more than the gap.
       */
      rebased?: boolean
    }
...
export async function runRebaseCycle(ctx: RebaseContext): Promise<RebaseSummary> {
  ...
  for (const branchDir of branchDirs) {
    ...
    // skip logs below don't fire for them every cycle; everything else still
    // logs loudly on skip.
    ...
    // refreshBaseBranchWorkspace() earlier in syncGit(). Routing it through
    // this conflict-resolution loop could rewrite its history (the --theirs
    // loop below) and stamp meaningless conflict metadata on it. Compare
    // sanitized-vs-sanitized: branchDir is a filesystem name, ctx.baseBranch
    // is raw.
...
async function rebaseOneBranch(
  ...
  try {
    ...
    // Skip branches that must not be mutated:
    // - submitted/approved: in review, so never rewrite history under an active
    //   PR -- but do poll GitHub for the PR's resolution, since nothing else
    //   tells the worker a merge/close happened;
    // - archived: already merged, with no PR left to poll.
    ...
    // check-then-act, and its residual window spans fetch, replay and N
    // conflict rounds of awaited git subprocesses on EFS. A save landing in
    // there is destroyed two ways: `checkout --theirs` below overwrites the
    // just-saved file with the branch's committed version and the rebase then
    // SUCCEEDS (nothing logs a failure at all), and `rebase --abort`
    ...
    // skipped and retried next sync cycle (~5 min), the same principle as the
    // skip-dirty-branches behaviour below. Writers get the patient side of the
    // asymmetry -- see utils/content-write-lock.ts.
    //
    // The heartbeat keeping this lock fresh (proper-lockfile refreshes every
    // `stale`/2 = 15s) is a timer on this event loop, and every git step below
    // is an awaited subprocess rather than a synchronous block, so the refresh
    // keeps firing for the whole hold.
    ...
    try {
      // Recover an INTERRUPTED rebase before anything else looks at this tree.
      // A clone left with .git/rebase-merge (or rebase-apply) reports
      // uncommitted changes, so without this the dirty check below classifies
      // it `skippedDirty` on every cycle FOREVER while branch-health scans it
      // as healthy -- and editors meanwhile read, and can save over,
      // conflict-marker content. An in-progress rebase here is always this
      // worker's own abandoned work (nothing else rebases these clones) left by
      // a crash, an OOM, a spot interruption, or the ASG rolling the instance.
      //
      // NOT LOSSLESS, and it is important not to claim otherwise. `git rebase
      // --abort` hard-resets tracked files to the pre-rebase head. While the
      // worker was DOWN nothing held the [SYNC-C1] content-write lock, so an
      // editor could have saved into this wedged clone and received a 200; that
      // save is a working-tree modification and the abort reverts it (new,
      // untracked entry files survive; edits to existing ones do not). Taking
      // the lock here stops any FURTHER save racing the abort but cannot
      // recover one that already landed.
      //
      // Aborting anyway is still right -- the alternative is a branch wedged
      // forever serving conflict-marker content to editors -- but it must not
      ...
      if (await isRebaseInProgress(branchPath)) {
        ...
        //                            merged files, already STAGED: committed
        //                            history that survives the abort untouched.
        //   ` M` index=' ', wd=M  -- an unstaged working-tree modification: an
        //                            editor's save landing while the worker was
        //                            down. The abort discards exactly these.
        //   `??`                  -- untracked; the abort leaves them.
        //
        // KNOWN GAP: a save onto one of the rebase's own conflicted paths (the
        // "saved over conflict-marker content" case) is excluded below, because
        // the file reads `UU` whether or not an editor touched it and status
        // alone cannot tell the two apart. Those discards go unlogged.
        ...
        try {
          ...
          // (`!completed` and the outer catch): without a `syncFailureReason`
          // the admin branch panel shows nothing for a persistently
          // un-abortable wedge, which is precisely the state needing an
          // operator, since the next cycle cannot fix it by itself.
      ...
      // need not match it. A literal 'HEAD' means a detached clone (a crashed
      // rebase can leave one), where nothing below can safely name a ref, so
      // every publish path stays disarmed -- the safe direction.
      ...
      // branch is not behind base. Every crash window in the arming sequence
      // below leaves the marker set with the work unfinished, and this is what
      // completes it -- without it one lost lease race strands the branch until
      // the base happens to advance again. Gated on the loop-top snapshot, so
      // unmarked branches (nearly all of them, every cycle) cost nothing.
      ...
      if (behindCount === 0) {
        // Already in sync -- the overwhelmingly common outcome per branch per
        // cycle, so skip the save entirely when metadata already reflects a
        // clean state. save() eager-regenerates the branch registry
        // (branch-metadata.ts's invalidateRegistry(), O(branch count) EFS
        // reads), so an unconditional save here makes every rebase cycle
        // O(N^2) registry work for a true no-op. Re-load fresh, not the
        // `metaFile` snapshot from before the fetch/rev-list above, so a
        // concurrent editor-driven metadata change in that window is not
        // clobbered by a stale skip decision.
        const currentMeta = await BranchMetadataFileManager.loadOnly(branchPath)
        ...
        // A lingering rebaseFailure must also be cleared once the branch
        // catches up clean, or it sticks as a stale warning forever: nothing
        // else touches a caught-up branch, so no other save site clears it.
      ...
      // Read BOTH sides before rewriting anything: the arming guard after the
      // rebase compares what remote.git published against what this clone is
      // about to rebase away, and the clone's pre-rebase tip is exactly what
      ...
      // wrong with the branch, we simply can no longer prove we were the only
      // writer. Handled ahead of the `!completed` block so it never records a
      // user-visible rebaseFailure or lands in `failed[]`.
      //
      // ONLY when the rebase did not complete. Bailing out of a COMPLETED
      // rebase is worse than useless: the history is already rewritten (so
      // `--abort` is a no-op), and skipping the completion path below strands
      // three things the next cycle never redoes, because a caught-up branch
      // short-circuits at the `behindCount === 0` check above -- the [SYNC-H1]
      // `markHistoryRewritten` marker (without which the editor's next submit
      // is rejected non-fast-forward and mis-diagnosed as another deployment),
      // the content-cache invalidation for a tree that DID change, and the
      // conflictStatus save. That turns a transient lock compromise into a
      // permanently wedged published branch.
      //
      ...
      if (!completed) {
        // failureReason is only set on the "unexpected error" break above;
        // MAX_REBASE_ROUNDS exhaustion is a distinct exit with no message of
        // its own, so the warn text must not conflate the two.
        workerLogWarn(
        ...
        // [REDACT] failed[] folds into worker-status.json's lastGitSync.failed,
        // served to the browser -- failureReason can be an arbitrary git error
        // message that embeds the bot token.
        const redactedRebaseFailureMessage = redactCredentials(rebaseFailureMessage)
        // Record ONCE here for the "!completed" exit: the unexpected-error
        // break above always falls through to this block, so recording at the
        // break itself would double-record. The outer catch below is the only
        // other record site, covering a disjoint failure class -- errors
        // outside the round loop, e.g. fetch/rev-list failures.
      ...
      // The rebase rewrote the clone's working tree — mark ContentStore ID
      // indexes rooted here stale so lookups rebuild from disk, in this process
      // and (via the on-disk generation marker) in the Lambda containers
      ...
      await meta.save({
        branch: {
          ...
          // The cycle completed, so clear any prior failure record regardless
          // of the conflict outcome.
          rebaseFailure: undefined,
        },
      })
      didRebase = true
      ...
      // Last-resort guarantee that NO exit path leaves this clone mid-rebase,
      // including an unexpected throw from any git step above.
      //
      // It must live HERE rather than in the outer catch: that catch runs AFTER
      // this finally has released the content-write lock, so aborting there
      // would hard-reset a working tree an editor's save could already be
      // racing -- precisely the [SYNC-C1] hazard the lock exists to prevent.
      // Inside the finally the lock is still held, EXCEPT on the narrow path
      // where it was compromised mid-hold, where a newly-admitted writer may be
      // live and this abort carries the same exposure as the compromise path's
      // own abort above. Not special-cased: leaving a clone wedged mid-rebase
      // is worse, and that writer is already being told to retry.
      //
      // Guarded on actual rebase state, so the happy path and the early returns
      // cost one stat and do nothing.
      try {
        if (await isRebaseInProgress(branchPath)) {
          workerLogWarn(
      ...
      // [SYNC-C1] Released on EVERY exit -- the early returns above, a throw
      // into the outer catch, and the happy path alike. A stranded lock would
      // wedge every write to this branch until it went stale.
    ...
    // [REDACT] Same rationale as the `if (!completed)` site above: this catches
    // fetch/rev-list/unexpected errors, whose message can embed the bot token.
    const redactedMessage = redactCredentials(message)
    // Second and only other record site -- see the `if (!completed)` block
    // above for why the two are disjoint.
