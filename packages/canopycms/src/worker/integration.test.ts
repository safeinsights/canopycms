...
describe('Worker integration: task queue + auth cache', () => {
  ...
  describe('auth cache: write → read via CachingAuthPlugin', () => {
    it('CachingAuthPlugin reads from file cache populated by worker', async () => {
      ...
      await fs.writeFile(path.join(cachePath, 'users.json'), JSON.stringify({
        users: [
          { id: 'user_1', name: 'Alice', email: 'alice@test.com', avatarUrl: 'https://avatar/alice' },
          { id: 'user_2', name: 'Bob', email: 'bob@test.com' },
        ],
      }))
      await fs.writeFile(path.join(cachePath, 'orgs.json'), JSON.stringify({
        groups: [
          { id: 'org_1', name: 'Engineering', memberCount: 5 },
        ],
      }))
      await fs.writeFile(path.join(cachePath, 'memberships.json'), JSON.stringify({
        memberships: {
          user_1: ['org_1'],
          user_2: [],
        },
    ...
    it('cache updates are picked up on next read', async () => {
      ...
      await fs.writeFile(path.join(cachePath, 'memberships.json'), JSON.stringify({ memberships: {} }))
  ...
  describe('end-to-end: task queue + auth cache together', () => {
    it('simulates full prod-sim workflow', async () => {
      ...
      await fs.writeFile(path.join(cachePath, 'users.json'), JSON.stringify({
        users: [
          { id: 'devuser_2nK8mP4xL9', name: 'User One', email: 'user1@localhost.dev' },
        ],
      }))
      await fs.writeFile(path.join(cachePath, 'orgs.json'), JSON.stringify({
      ...
      const devVerifier: TokenVerifier = async () => ({ userId: 'devuser_2nK8mP4xL9' })
