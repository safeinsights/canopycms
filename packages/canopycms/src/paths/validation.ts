 ...
 * ```
 */
export function parseSlug(slug: string, type: 'collection' | 'entry'):
  | { ok: true; slug: CollectionSlug | EntrySlug }
  | { ok: false; error: string } {
  if (!slug || typeof slug !== 'string') {
    return { ok: false, error: `${type === 'collection' ? 'Collection' : 'Entry'} slug is required` }
  }
  ...
  if (!/^[a-z0-9][a-z0-9-]*$/.test(slug)) {
    return {
      ok: false,
      error: 'Slug must start with a letter or number and contain only lowercase letters, numbers, and hyphens'
    }
  }
