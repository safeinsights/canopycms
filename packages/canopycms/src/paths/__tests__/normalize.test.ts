...
describe('createLogicalPath', () => {
  ...
  it('throws on traversal sequence', () => {
    expect(() => createLogicalPath('content', '..', 'evil')).toThrow('Invalid path: contains traversal sequence')
  })
})

describe('createPhysicalPath', () => {
  it('joins segments with forward slashes', () => {
    expect(createPhysicalPath('content', 'posts', 'my-post.ABC123.mdx')).toBe('content/posts/my-post.ABC123.mdx')
  })

  it('throws on traversal sequence', () => {
    expect(() => createPhysicalPath('content', '../evil')).toThrow('Invalid path: contains traversal sequence')
  })
})
