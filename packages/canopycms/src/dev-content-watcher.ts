 ...
 * A developer editing working-tree `content/**` outside the editor therefore gets stale content
 * from the dev server until `canopycms sync push` runs. This watcher reports that divergence
 * ('warn', the default: a warning naming the diverged files, at startup and on content changes) or
 * does nothing ('off').
 *
 * There is intentionally NO auto-push mode: overwriting the branch clone from the working tree
 * would clobber uncommitted editor "Save" state with no Canopy-level recovery. Reconciliation goes
 * through the interactive, conflict-aware `canopycms sync push` instead.
 *
 * Divergence is a CONDITION, not an event: it holds until someone runs `sync push`, so the report
 * must not repeat into a scrolling request log (`reportOnce`) yet must be hard to scroll past (the
 * gutter block). A log cannot hold a condition; see
 * `.claude/future-tasks/dev-divergence-in-app-surface.md` for the in-app surface that would
 * replace the terminal as its primary home.
 *
 * All logic lives here; framework adapters call startDevContentWatcher() once at dev startup (see
 * packages/canopycms-next/src/context-wrapper.ts for the Next wiring).
 */
 ...
 * not a diff viewer, and a wall of paths is as easy to scroll past as a one-line warning. Each
 * list carries its true count, so nothing is hidden -- only elided.
 */
 ...
 * and immediately arms a new one whose startup check re-finds the same divergence, so dropping the
 * state with the watcher is what re-prints the identical block.
 */
...
/**
 * The registry lives on `globalThis`, NOT in module scope: Next's dev server compiles the server
 * graph per route bundle and evaluates each copy in its own module scope, so a module-level `Map`
 * is re-created empty per bundle -- every copy then believes it is the first watcher, arms its own,
 * and re-prints the startup warning. A `globalThis` property is the one thing shared across those
 * evaluations.
 */
...
/**
 * One body line of the notice block. A left gutter rather than a full box: content paths are long
 * and unpredictable, and a right border would force either wrapping math or truncation of the very
 * filenames the block exists to name.
 */
...
function formatDivergenceWarning(branch: string, diff: ContentTreeDiff): string {
  ...
  // Blank lines top and bottom: whitespace separation does more to make this block findable in a
  // scrolling request log than any amount of in-block decoration.
  return [
    '',
...
export function startDevContentWatcher(
  ...
  // Pass the resolved contentRoot local above: getContentRoot has no default of its own, by
  // design -- a default here silently no-ops this watcher for a non-default contentRoot.
  ...
  // Resolve the branch fresh each check: the dev server follows git-HEAD switches
  // (services.refreshActiveBranch reassigns config.defaultActiveBranch), so the comparison target
  // must track whatever branch is currently served. An explicit options.branch pins it.
  const resolveBranch = () =>
    options.branch ??
  ...
  /**
   * Emit `message` unless it is verbatim what was emitted last for this directory -- divergence
   * block, resolved notice and error paths alike, since in every case a repeat means "the
   * condition is unchanged", which the reader already knows.
   */
  ...
  const check = async (): Promise<void> => {
    ...
    try {
      ...
      if (isContentTreeDiffEmpty(diff)) {
        // An announced condition needs its retraction announced too, or the reader is left
        // believing the dev server is still serving stale content.
  ...
  // Watcher errors (e.g. inotify ENOSPC/EMFILE) must be handled, or an unhandled 'error' event
  // crashes dev.
