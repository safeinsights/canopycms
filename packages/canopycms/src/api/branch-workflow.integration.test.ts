...
describe('PR Workflow Integration', () => {
  ...
  it('completes full PR workflow: create → submit → withdraw → request changes → resubmit → merge', async () => {
    ...
    const submittedContext = await loadBranchContext({ branchName, mode: config.mode ?? 'local-simple' })
    ...
    const withdrawnContext = await loadBranchContext({ branchName, mode: config.mode ?? 'local-simple' })
    ...
    const changesRequestedContext = await loadBranchContext({ branchName, mode: config.mode ?? 'local-simple' })
    ...
    const resubmittedContext = await loadBranchContext({ branchName, mode: config.mode ?? 'local-simple' })
    ...
    const archivedContext = await loadBranchContext({ branchName, mode: config.mode ?? 'local-simple' })
