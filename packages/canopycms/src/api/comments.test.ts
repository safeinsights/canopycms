...
describe('comments api - addComment', () => {
  it('returns 404 if branch not found', async () => {
    const ctx = makeCtx()
    ctx.getBranchState = vi.fn().mockResolvedValue(null)
    const res = await addComment(ctx, { user: { userId: 'u1' }, body: { text: 'test' } }, { branch: 'missing' })
    expect(res.status).toBe(404)
  })
  ...
  it('returns 400 if text is missing', async () => {
    const res = await addComment(makeCtx(), { user: { userId: 'u1' }, body: {} as any }, { branch: 'feature/x' })
