...
describe('comments api - listComments', () => {
  it('returns 404 if branch not found', async () => {
    ...
    const res = await listComments(ctx, { user: { type: 'authenticated', userId: 'u1', groups: [] } }, { branch: unsafeAsBranchName('missing') })
    expect(res.status).toBe(404)
  })

  it('returns 403 if access forbidden', async () => {
    const res = await listComments(makeCtx(false), { user: { type: 'authenticated', userId: 'u1', groups: [] } }, { branch: unsafeAsBranchName('feature/x') })
    expect(res.status).toBe(403)
  })

  it('lists comments when allowed', async () => {
    const res = await listComments(makeCtx(), { user: { type: 'authenticated', userId: 'u1', groups: [] } }, { branch: unsafeAsBranchName('feature/x') })
...
describe('comments api - addComment', () => {
  it('returns 404 if branch not found', async () => {
    ...
    const res = await addComment(
      ...
      { text: 'test', type: 'field', entryPath: unsafeAsLogicalPath('posts/hello'), canopyPath: 'title' }
    )
    expect(res.status).toBe(404)
  })
  ...
  it('adds field comment when allowed', async () => {
    const res = await addComment(
      ...
      { user: { type: 'authenticated', userId: 'u1', groups: [] } },
      { branch: unsafeAsBranchName('feature/x') },
      { text: 'Great work!', type: 'field', entryPath: unsafeAsLogicalPath('posts/hello'), canopyPath: 'title' }
    )
    expect(res.ok).toBe(true)
