...
export const deleteBranchHandler = async (
  ...
  try {
    ...
    if (!isNotFoundError(err)) {
      console.error(`CanopyCMS: Failed to delete branch metadata for ${branchName}:`, getErrorMessage(err))
    }
  }
  ...
  if (branchContext.branchRoot !== branchContext.baseRoot) {
    try {
      ...
      console.error(`CanopyCMS: Failed to delete branch directory for ${branchName}:`, getErrorMessage(err))
    }
  }
