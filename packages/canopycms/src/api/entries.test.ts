...
describe('listEntries', () => {
  ...
  it('lists entries recursively from deeply nested collections', async () => {
    ...
    await fs.mkdir(path.join(root, `content/docs.${docsId}/api.${apiId}/v2.${v2Id}`), { recursive: true })
  ...
  it('lists root-level entry types with maxItems: 1', async () => {
    ...
    const homeEntry = res.data?.entries.find((e) => e.slug === 'home' && e.collectionPath === 'content')
    expect(homeEntry).toBeDefined()
    ...
    const settingsEntry = res.data?.entries.find((e) => e.slug === 'settings' && e.collectionPath === 'content')
