...
export const EditorPanes: React.FC<EditorPanesProps> = ({
  ...
  const TypedSplitPane = SplitPane as unknown as React.ComponentType<React.PropsWithChildren<SplitPaneProps>>
  ...
  const primarySize = useMemo(() => (layout === 'side' ? sidePrimarySize : stackedPrimarySize), [layout, sidePrimarySize, stackedPrimarySize])
  ...
  const updateSizeFromPixels = (nextPixels: number, nextLayout: PaneLayout) => {
    const total =
      nextLayout === 'side' ? splitContainerRef.current?.clientWidth ?? 0 : splitContainerRef.current?.clientHeight ?? 0
  ...
  return (
    <Box h="100%" style={{ minHeight: '70vh' }}>
      ...
      >
        <Box ref={splitContainerRef} style={{ display: 'flex', flex: 1, minHeight: 0, width: '100%' }}>
          <TypedSplitPane
            split={direction}
            minSize={120}
            size={`${primarySize}%`}
            ...
            style={{ position: 'relative', minHeight: 0, width: '100%', height: '100%', userSelect: isDragging ? 'none' : undefined }}
