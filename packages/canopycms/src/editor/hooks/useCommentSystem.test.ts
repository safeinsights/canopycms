...
describe('useCommentSystem', () => {
  ...
  const mockComments: CommentThread[] = [
    ...
    },
    {
      id: 'thread3',
      type: 'branch',
      comments: [{ id: 'c3', threadId: 't3', text: 'Branch comment', userId: 'user1', timestamp: new Date().toISOString() }],
      resolved: false,
      createdAt: new Date().toISOString(),
      authorId: 'user1',
    },
  ...
  it('resolves thread successfully', async () => {
    ...
    expect(mockClient.comments.resolve).toHaveBeenCalledWith({ branch: 'main', threadId: 'thread1' })
    // Branch summaries auto-update via useMemo watching comments
    // No need to reload branches explicitly
  })
