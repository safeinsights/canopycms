 ...
 * ```
 */
export function setupMockConsole(methods: Array<'error' | 'warn' | 'log' | 'info' | 'debug'> = ['error']) {
  const spies: any = {}
  ...
  return {
    ...spies,
    restore,
  } as Record<'error' | 'warn' | 'log' | 'info' | 'debug', ReturnType<typeof vi.spyOn>> & { restore: () => void }
}
