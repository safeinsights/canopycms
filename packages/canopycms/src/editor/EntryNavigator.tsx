...
export const EntryNavigator: React.FC<EntryNavigatorProps> = ({
  ...
  const renderNode = ({ node, elementProps, hasChildren, expanded, level }: RenderTreeNodePayload) => {
    ...
    const canReorder = onReorderEntry && contentId && parentCollectionPath && typeof childIndex === 'number'
    const canMoveUp = canReorder && childIndex > 0
    const canMoveDown = canReorder && typeof totalChildrenCount === 'number' && childIndex < totalChildrenCount - 1
