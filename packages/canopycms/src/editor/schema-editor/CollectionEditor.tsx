...
import type { CreateCollectionInput, UpdateCollectionInput, CreateEntryTypeInput } from '../../schema/schema-store-types'
...
export interface CollectionEditorProps {
  ...
  onUpdateEntryType?: (collectionPath: LogicalPath, entryTypeName: string, updates: Partial<CreateEntryTypeInput>) => void
...
export function CollectionEditor({
  ...
  const [deletingEntryType, setDeletingEntryType] = useState<{ entryType: ExistingEntryType | CreateEntryTypeInput; index: number } | null>(null)
  ...
  // Update a form field
  const updateField = useCallback(<K extends keyof CollectionFormData>(
    field: K,
  ...
  const handleEntryTypeSave = useCallback((data: CreateEntryTypeInput | Partial<CreateEntryTypeInput>, isNew: boolean) => {
    ...
    setEditingEntryType(null)
    setEditingEntryTypeIndex(null)
  }, [isEditMode, editingCollection, editingEntryType, editingEntryTypeIndex, onAddEntryType, onUpdateEntryType])

  const handleRemoveEntryType = useCallback((entryType: ExistingEntryType | CreateEntryTypeInput, index: number) => {
  ...
  return (
    <>
          ...
          {displayEntryTypes.length === 0 ? (
            <Paper p="md" withBorder>
              <Text c="dimmed" ta="center" size="sm">
                No entry types defined. Add at least one entry type to define what content can be created in this collection.
              </Text>
            </Paper>
          ) : (
            <Stack gap="xs">
                ...
                <Paper key={entryType.name} p="sm" withBorder>
                  <Group justify="space-between" wrap="nowrap">
                    <Group gap="sm">
                      <div>
                        <Group gap="xs">
                          <Text fw={500} size="sm">{entryType.name}</Text>
                          {entryType.default && (
                          ...
                          {entryType.maxItems === 1 && (
                            <Badge size="xs" color="gray">Singleton</Badge>
                          )}
                        </Group>
                    ...
                    <Menu position="bottom-end" withinPortal>
                      ...
                      <Menu.Dropdown>
                        <Menu.Item
                          ...
                          onClick={() => handleOpenEditEntryType(entryType as ExistingEntryType, index)}
                        >
                          Edit
                        </Menu.Item>
                        <Menu.Divider />
      ...
      <ConfirmDeleteModal
        ...
        message={
          deletingEntryType && 'usageCount' in deletingEntryType.entryType && deletingEntryType.entryType.usageCount
