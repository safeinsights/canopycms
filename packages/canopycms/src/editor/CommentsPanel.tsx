  ...
  const handleAddComment = async () => {
    ...
    try {
      if (replyTo) {
        ...
        if (thread) {
          await onAddComment(newCommentText, thread.type, thread.entryId, thread.canopyPath, replyTo)
        }
      } else {
              ...
              {filteredThreads.map((thread) => (
                <Paper key={thread.id} withBorder p="md" bg={thread.resolved ? 'gray.0' : undefined}>
                  <Stack gap="xs">
                    ...
                    <Group gap="xs">
                      {thread.type === 'field' && thread.entryId && thread.canopyPath && onJumpToField && (
                        <Button
                          size="xs"
                          variant="light"
                          onClick={() => {
