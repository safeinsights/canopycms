...
const ROUTE_SEP = '|'
const routeKey = (method: string, segments: string[]) =>
  `${method.toUpperCase()}${ROUTE_SEP}${segments.join('/')}`

const buildRouteMap = (options: CanopyNextOptions): Record<string, CanopyRouteHandler> => {
  const withOptions = <T extends CanopyNextHandler>(handler: T) =>
    adaptCanopyHandler(handler, options)
  ...
  return {
    ...
    [routeKey('POST', [':branch', 'comments', ':threadId', 'resolve'])]:
      withOptions(resolveComment),
...
export const createCanopyCatchAllHandler = (options: CanopyNextOptions = {}) => {
  ...
  return async (
    req: NextRequest,
    ctx?: { params?: { canopycms?: string[]; [key: string]: any } },
  ) => {
...
export const canopyHandlers = {
  ...
  submitBranchForMerge: (options?: CanopyNextOptions) =>
    adaptCanopyHandler(submitBranchForMerge, options),
