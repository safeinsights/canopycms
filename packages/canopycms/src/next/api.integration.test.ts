...
describe('canopycms catch-all integration', () => {
  ...
  it('drives branch create, content read/write, entry listing, and submit via the catch-all route', async () => {
    ...
    const call = async (method: string, segments: string[], body?: unknown, query?: Record<string, string>) => {
    ...
    const preSubmitState = await loadBranchState({ branchName, mode: config.mode ?? 'local-simple' })
    ...
    const readPost = await call('GET', [branchName, 'content', encodeId(postsCollectionId), 'hello-world'])
    ...
    const remoteHome = JSON.parse(await fs.readFile(path.join(verifyPath, 'content/home.json'), 'utf8'))
