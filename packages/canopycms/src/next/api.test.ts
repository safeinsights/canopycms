...
vi.mock('../api/branch-status', () => ({
  ...
  submitBranchForMerge: vi.fn().mockResolvedValue({ ok: true, status: 200, data: { branch: { branch: { name: 'x' } } } }),
}))
vi.mock('../api/content', () => ({
...
describe('Next API adapter', () => {
  const mockAuthPlugin = createMockAuthPlugin({ userId: 'test-user', groups: [RESERVED_GROUPS.ADMINS] })
