...
describe('SchemaCacheRegistry', () => {
  ...
  describe('prod-sim mode', () => {
    it('should load schema from .collection.json files on first access (cache miss)', async () => {
      ...
      const result = await registry.getSchema(branchRoot, schemaRegistry)
    ...
    it('should use cache on second access (cache hit)', async () => {
      ...
      const result1 = await registry.getSchema(branchRoot, schemaRegistry)
      const duration1 = Date.now() - start1

      // Second access - should be faster (cache hit)
      const start2 = Date.now()
      const result2 = await registry.getSchema(branchRoot, schemaRegistry)
    ...
    it('should write cache file to .canopy-meta/schema-cache.json', async () => {
      ...
      await registry.getSchema(branchRoot, schemaRegistry)
