...
- **Turbopack guard (Next 16+)** — On Next 16 and later, sets `turbopack: {}` if your config has neither `turbopack` nor your own `webpack`, since Next 16 defaults `next build`/`next dev` to Turbopack and exits when it sees the React-aliasing `webpack` function above with no `turbopack` config. Your own `webpack` or `turbopack` config is always left as-is, so a `turbopack: {}` you already added keeps working.
