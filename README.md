...
- **Node.js**: `>=22.12.0`, both to consume the published packages and to work in this monorepo (`.nvmrc` pins the major). The packages are ESM-only and reach CommonJS consumers through `require(esm)`, which Node unflagged in 22.12.0 — so a CommonJS project (a stock `cdk init` app, for instance) cannot load them on anything earlier.
...
- **Turbopack guard (Next 16+)** — On Next 16 and later (or when the installed Next version can't be read), sets `turbopack: {}` if your config has neither `turbopack` nor your own `webpack`, since Next 16 defaults `next build`/`next dev` to Turbopack and exits when it sees the React-aliasing `webpack` function above with no `turbopack` config. Your own `webpack` or `turbopack` config is always left as-is.
...
| `tsconfig.json`                    | adds `infrastructure` to `exclude` so `next build` doesn't type-check the CDK app; left alone with a warning if the file has comments or doesn't exist                                                                                                                                                                                                             |
