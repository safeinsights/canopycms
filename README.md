...
| `defaultBaseBranch`   | `string`               | No       | `'main'`    | Git branch used as the fork point for CMS content branches. In dev mode, auto-detected from git HEAD if not set                                                                                          |
...
- **`dev`**: Full-featured local development with branching and git operations. Uses a local bare remote at `.canopy-dev/remote.git` and branch workspaces at `.canopy-dev/content-branches/`. Both `defaultBaseBranch` and `defaultActiveBranch` are auto-detected from the current git branch if not set (e.g., if you are on `feat-bar`, the dev server and editor default to that branch). Add `.canopy-dev/` to `.gitignore`.
