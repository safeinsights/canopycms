...
| context.ts               | Context creation and management; exports `CanopyBuildContext` (build-time: buildContentTree, listEntries, services) and `CanopyContext extends CanopyBuildContext` (adds read, readByUrlPath, user); schema resolution memoized per call; readByUrlPath uses ContentStoreError.code for probing instead of regex message matching                                                                                                                                                                                                                                                                    |
...
| sync.ts                | Bidirectional content sync between working tree and branch workspaces; push copies+commits to workspace, pull copies back, both does 3-way git merge via canopycms-sync-base tag, abort cancels a failed merge; includes `assertWithinDir` path-traversal guard, `safeReplaceDir` crash-safe directory replacement, `selectBranch` interactive branch picker (accepts `autoCreate` to auto-create workspaces on push), `detectCurrentBranch` helper |
...
| File                   | Purpose                                                                                                                                                                                                                                                                               |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| types.ts               | TypeScript type definitions for all config; ReferenceFieldConfig has optional `collections` and optional `entryTypes`; includes `entryLinkUrl?: EntryLinkUrlResolver` callback; InlineGroupFieldConfig (`type: 'group'`) is a FieldConfig union member for visual-only field grouping |
