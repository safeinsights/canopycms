...
| content-index-generation.ts | On-disk cross-process generation marker for the ContentId index at `{root}/.canopy-meta/content-index.generation` (shared filesystem — e.g. Lambda containers + EC2 worker on EFS — is the coordination medium; random token per bump instead of a counter, atomic rename; bumpers write AFTER mutations, readers capture BEFORE scanning): `bumpContentIndexGeneration`/`readContentIndexGeneration`; `invalidateContentIndexesDurable(root)` is the single entry point mutation sites call (marker bump + in-process registry invalidation)                                                                                                                                                                                                                                                                                                                                                  |
...
| content-index-generation.ts | On-disk cross-process generation marker (`{root}/.canopy-meta/content-index.generation`); `invalidateContentIndexesDurable(root)` = marker bump + in-process registry invalidation — the single entry point for mutation sites                                                                                                                                                                                    |
