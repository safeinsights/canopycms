...
| sync-core.ts             | Prompt-free core of working-tree↔branch-clone content sync (extracted from cli/sync.ts, which now imports it; reused by dev-content-watcher.ts). Exports `assertWithinDir`, `safeReplaceDir`, `listFilesRecursive`, `copyDir`, `diffContentTrees` (byte comparison, not mtime), `isContentTreeDiffEmpty`, `pushContentToWorkspace` (copy + commit + optional sync-base tag), `SYNC_BASE_TAG` (`'canopycms-sync-base'`); types `ContentTreeDiff`/`PushContentToWorkspaceOptions`                                                                                                                                                                                                                                                                                                                                  |
| dev-content-watcher.ts   | `startDevContentWatcher(services, options)`: dev-mode chokidar watcher that detects divergence between working-tree `content/**` and the served branch clone. Per `dev.contentSync`: 'warn' (default) logs which files diverged, 'auto' pushes working-tree content into the clone via `pushContentToWorkspace`, 'off' is a no-op. No-op outside dev mode or when the working-tree content dir is absent; returns a disposer                                                                                                                                                                                                                                                                                                                                                                                     |
...
| context-wrapper.ts | `createNextCanopyContext()` returns `NextCanopyContextResult` with: `getCanopy` (request-scoped via React `cache()` + `headers()`), `getCanopyForBuild` (returns `CanopyBuildContext` — narrower type, no `user`; now surfaces build-safe `read`/`readByUrlPath` alongside buildContentTree/listEntries/services; uses `STATIC_DEPLOY_USER`, memoized per process, safe for `generateStaticParams`/`generateMetadata`; guarded by `guardBuildContext` so its reads throw if used at request time on a `server` deployment), phase-selecting `readByUrlPath`/`read` (build context at build via `isBuildMode()`, runtime context at request — recommended for `[...slug]`/`[slug]` page resolution), `handler` (API catch-all), `services`. Starts the dev content watcher (`startDevContentWatcher`) when mode === 'dev' (not in build mode) |
| static.ts          | `generateContentStaticParams(getCanopyForBuild, opts)` — Next adapter over `collectStaticPaths`; emits the array Next's `generateStaticParams` expects. `shape: 'catch-all'` (default, param = URL `segments`) or `'single'` (param = entry `slug`, pair with `rootPath` to scope a collection); `paramName` (default `'slug'`); `GenerateContentStaticParamsOptions extends CollectStaticPathsOptions`. Exported from index.ts                                                                                                                                                                                                                                                                                                                                                                                                              |
...
Dev-mode-only setting controlling how working-tree `content/**` edits made outside the editor are reconciled with the served branch clone. Ignored when `mode !== 'dev'`.

| Value              | Behavior                                                                              |
| ------------------ | ------------------------------------------------------------------------------------- |
| `'warn'` (default) | Log a warning naming the diverged files (on startup and on content changes)           |
| `'auto'`           | Push working-tree content into the branch clone automatically (no manual `sync push`) |
| `'off'`            | No watcher, no warnings                                                               |

Implemented by `startDevContentWatcher()` (src/dev-content-watcher.ts), wired once at dev startup by `createNextCanopyContext()` (canopycms-next) via `dev.contentSync`. Builds on sync-core.ts (`diffContentTrees`, `pushContentToWorkspace`).
