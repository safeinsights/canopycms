...
| branch-metadata.ts       | Branch metadata with layered concurrency safety (in-process `withLock` + server-enforced `withOccFileLock` + OCC via `utils/occ-json-write.ts`, retry-on-conflict); `save()` treats `baseBranch` as immutable after creation (existing value always wins); see docs/concurrency.md                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
