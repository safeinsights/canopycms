...
| /api/canopycms/schema             | schema.ts             | Schema CRUD (collections, entry types, ordering) - admin only; GET /:branch/schema sends flat `WireFlatSchemaItem[]` — collection items carry NO nested `collections` subtree on the wire (children linked via `parentPath`; embedded `entries`/`order` kept); in-memory `FlatSchemaItem.collections` unchanged; single-collection GET (getCollection) still returns a nested subtree                                                        |
...
| migrate.ts             | `canopycms migrate` command: converts a plain content tree to CanopyCMS conventions — ID-based entry renames (`{type}.{slug}.{id}.{ext}`), directory renames with embedded IDs, `.collection.json` scaffolding; idempotent (already-conforming names skipped), `--dry-run` supported; exports `migrate()`, `MigrateError`, `slugifyName`, `MIGRATE_FORMATS` (`md`/`mdx`/`json`/`yaml`)                                                                                                                                                                                                                                                                                                                                                                     |
