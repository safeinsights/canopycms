...
2. **`trailingSlash` is an explicit option, and it is not inferred.** CanopyCMS cannot see your
...
3. **`noindex` drives BOTH surfaces from one predicate** — `robots: { index: false }` on the
...
exactly one component per template name — no more, no fewer. Deliberately shipped as
