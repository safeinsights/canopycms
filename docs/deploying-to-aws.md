...
**Known limitation:** raw `FormData` bodies (multipart uploads) can't be hashed this way — the multipart boundary is generated by the runtime at send time, so the final bytes aren't known until after the header would need to be set. No current CanopyCMS endpoint sends a `FormData` body (asset upload sends a JSON-encoded `Buffer`/`Uint8Array`), but if a future asset-upload path switches to raw multipart `FormData`, it will 403 behind this OAC shape and will need a different approach (e.g. uploading raw bytes via an `ArrayBuffer`/`Blob` body, which CAN be hashed, instead of `FormData`).
