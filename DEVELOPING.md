...
It pins `LC_ALL=C.UTF-8`: `git grep -P` only compiles `\x{...}` escapes above 0xFF in PCRE2 UTF mode, which git enables only under a UTF-8 locale -- under `LC_ALL=C` the command dies with exit 128 instead of matching nothing. Both halves branch on exit status with `case` rather than `if` for exactly that reason: 0 (matches found) fails, 1 (clean) passes, anything else -- including that 128 -- fails loudly rather than being read as "clean". The filename half uses perl rather than `grep -P` so it is runnable on macOS too, where BSD grep has no `-P`, and it matches the raw UTF-8 **bytes** rather than decoding with `-CSD`: decoding makes a malformed-UTF-8 path a fatal match error, which would abort the scan partway and leave every later path unexamined — a way to mask a bidi filename, since invalid-UTF-8 paths are creatable on the runner's ext4. Detection there is signalled by output rather than exit status, because an `END` block runs on death too and would launder a fatal into a status the caller reads as clean. A wrong byte range would fail open, so the step self-tests the pattern against all nine codepoints before trusting a clean result. Those nine are the embeddings, overrides and isolates only — not the marks U+061C/U+200E/U+200F, which reorder just adjacent neutral runs and appear legitimately in RTL content this CMS edits. ESLint's `security/detect-bidi-characters` overlaps but is JS/TS-only and only a warning, so it doesn't gate CI.
