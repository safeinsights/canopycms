...
export class EditorPage {
  ...
  getFieldInput(fieldName: string): Locator {
    ...
    return this.formPane.locator(`input[data-canopy-field="${fieldName}"], textarea[data-canopy-field="${fieldName}"]`)
  }
  ...
  async waitForSaveNotification(): Promise<void> {
    await expect(
      this.page.locator('.mantine-Notification-root', { hasText: 'Saved' })
  ...
  async removeListItem(fieldName: string, index: number): Promise<void> {
    const removeButton = this.formPane.locator(`[data-testid="remove-list-item-${fieldName}-${index}"]`)
