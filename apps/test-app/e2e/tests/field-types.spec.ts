...
test.describe('Multi-Field Content Editing', () => {
  ...
  test.skip('textarea/MDX field: multi-line content', async ({ page }) => {
    ...
    const testPostPath = path.join(process.cwd(), 'apps/test-app/.canopy-prod-sim/content-branches/main/content/posts.qrstuvwxyz12/test-mdx-post.json')

    // Ensure posts directory exists
    await fs.mkdir(path.dirname(testPostPath), { recursive: true })

    // Create initial post content
    await fs.writeFile(testPostPath, JSON.stringify({
      title: 'Test MDX Post',
      author: 'Test Author',
      date: '2026-01-10',
      tags: [],
    ...
    // Verify persistence
    const content = await readContentFile<{ title: string; body: string }>('posts/test-mdx-post.json')
  ...
  test('multiple fields in single entry', async () => {
    ...
    const content = await readContentFile<{ title: string; tagline: string }>('home.home.bo7QdSwn9Tod.json')
