...
- Keep integration touchpoints between the example app and CanopyCMS to a minimum, e.g. `canopycms.config.ts`, the catch-all API route at `/api/canopycms/[...canopycms]`, and `app/lib/canopy.ts` (`getCanopy`/`readByUrlPath`/`read`, backed by `createNextCanopyContext`) for server-side content loads. The idea is that adopters of CanopyCMS only need to minimally change a few files in their code to achieve the integration. If a new connection to Canopy is needed, get approval first.
